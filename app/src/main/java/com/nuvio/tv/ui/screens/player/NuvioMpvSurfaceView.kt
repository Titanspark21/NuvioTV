...
class NuvioMpvSurfaceView @JvmOverloads constructor(
    ...
    private fun reloadAudioOutput() {
        ...
        if (reloaded) {
            // Recreating the audio output can discard live filters. Force the next
            // applyDialogueLeveler call to install the named filter again even when the
            // selected level did not change.
